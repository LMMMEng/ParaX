...
from models import vit_parax


def parse_parax_kernel_sizes_arg(value):
    if value is None:
        return None
    if isinstance(value, str):
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(
                'parax_kernel_sizes must be none or a Python list/tuple like [3, 5, 7] or (3,)') from exc
    if not isinstance(value, (list, tuple)):
        raise argparse.ArgumentTypeError(
            'parax_kernel_sizes must be none or a Python list/tuple like [3, 5, 7] or (3,)')
    return tuple(value)
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='vit_base_patch16_parax', type=str, metavar='MODEL',
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default=os.environ.get('CLS_DATA_ROOT', 'data'), type=str,
                        help='dataset path')
    ...
    parser.add_argument('--ffn_num', default=192, type=int, help='bottleneck middle dimension')
    ...
    parser.add_argument('--parax_rank', default=128, type=int, help='ParaX low-rank dimension')
    parser.add_argument('--parax_kernel_sizes', default=None, type=parse_parax_kernel_sizes_arg,
                        help='ParaX dynamic conv kernels, e.g. [3, 5, 7], (3,), or none')
    parser.add_argument('--parax_enable_conv', default=False, action='store_true',
                        help='Enable ParaX depthwise and routed convolution paths')
    parser.add_argument('--parax_router_hidden', default=16, type=int, help='ParaX router hidden dimension')
    parser.add_argument('--parax_ls_init_value', default=1.0, type=parse_optional_float_arg,
                        help='ParaX adapter layer-scale init value, or none')
    parser.add_argument('--parax_rs_init_value', default=1.0, type=parse_optional_float_arg,
                        help='ParaX adapter residual-scale init value, or none')
...
def main(args):
    ...
    if args.model.startswith('vit'):
        model = vit_parax.__dict__[args.model](
        num_classes=args.nb_classes,
        global_pool=args.global_pool,
        drop_path_rate=args.drop_path,
        tuning_config=tuning_config,
        parax_rank=args.parax_rank,
        parax_kernel_sizes=args.parax_kernel_sizes,
        parax_enable_conv=args.parax_enable_conv,
        parax_router_hidden=args.parax_router_hidden,
        parax_ls_init_value=args.parax_ls_init_value,
        parax_rs_init_value=args.parax_rs_init_value)
    else:
