...
class VSSBlock(nn.Module):
    ...
    def _forward_ssm(self, input: torch.Tensor):
        x = input
        if self.post_norm:
    ...
    def _forward_mlp(self, input: torch.Tensor):
        x = input
        if self.post_norm:
...
class ParaXPEFTVSSBlock(VSSBlock):
    def __init__(
        self,
        stage_depth=None,
        parax_rank=128,
        ...
        **kwargs,
        ...
        self.adapter1 = ParaXAdapter(**adapter_kwargs)
        self.adapter2 = ParaXAdapter(**adapter_kwargs)
        self.parax_enable_conv = parax_enable_conv

    def _apply_adapter(self, x, adapter, params):
        if self.channel_first:
            bsz, channels, height, width = x.shape
            tokens = rearrange(x, 'b c h w -> b (h w) c').contiguous()
            adapter_out = adapter(
                tokens,
                params,
                height if self.parax_enable_conv else None,
                width if self.parax_enable_conv else None,
            )
            return rearrange(adapter_out, 'b (h w) c -> b c h w', h=height, w=width).contiguous()

        bsz, height, width, channels = x.shape
        tokens = x.view(bsz, height * width, channels).contiguous()
        adapter_out = adapter(
            tokens,
            params,
            height if self.parax_enable_conv else None,
        ...
        return adapter_out.view(bsz, height, width, channels).contiguous()

    def forward(self, x, params=None):
        x = super()._forward_ssm(x)
        x = self._apply_adapter(x, self.adapter1, params)
        x = super()._forward_mlp(x)
        x = self._apply_adapter(x, self.adapter2, params)
        return x
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.parax_rank = parax_rank
        self.parax_kernel_sizes = tuple(parax_kernel_sizes)
        self.parax_enable_conv = parax_enable_conv
        self.parax_router_hidden = parax_router_hidden
        self.parax_ls_init_value = parax_ls_init_value
        self.parax_rs_init_value = parax_rs_init_value
        self.parax_force_fp16 = parax_force_fp16
        ...
        self.peft_params = nn.ModuleList()

        for i_layer in range(self.num_layers):
            stage_depth = depths[i_layer]
            dim = dims[i_layer]
            self.peft_params.append(nn.ParameterDict(parax_parameter_dict(
                dim=dim,
                rank=parax_rank,
                num_layers=stage_depth,
                parax_kernel_sizes=self.parax_kernel_sizes,
                enable_conv=self.parax_enable_conv,
            )))
    ...
    def set_only_parax_trainable(self, train_classifier=False):
        for name, param in self.named_parameters():
            if 'adapter' in name or 'peft_params' in name:
                param.requires_grad = True
            elif train_classifier and name.startswith('classifier'):
                param.requires_grad = True
            else:
    ...
    def _make_layer(
        ...
        **kwargs,
        ...
        for d in range(depth):
            blocks.append(ParaXPEFTVSSBlock(
                hidden_dim=dim,
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i, layer in enumerate(self.layers):
            for blk in layer.blocks:
                x = blk(x, params=self.peft_params[i])
    ...
    def flops(self, shape=(3, 224, 224), verbose=True):
        ...
        if flop_count is None or parameter_count is None:
            raise ImportError('fvcore is required to compute FLOPs for VMamba ParaX.')
...
VSSMParaX = VSSM
BackboneVSSMParaX = Backbone_VSSM


def _merge_defaults(base, overrides):
    merged = dict(base)
    merged.update(overrides)
    return merged


_PARAX_FINAL_DEFAULTS = dict(
    parax_rank=128,
    parax_kernel_sizes=(3, 5, 7),
    parax_enable_conv=True,
    parax_router_hidden=16,
...
def _build_vmamba_defaults(channel_first, **kwargs):
    defaults = dict(_PARAX_FINAL_DEFAULTS)
    defaults.update(kwargs)
    defaults['norm_layer'] = 'ln2d' if channel_first else 'ln'
    return defaults


def vmamba_tiny_parax(pretrained=None, channel_first=True, **kwargs):
    defaults = _build_vmamba_defaults(
        channel_first,
        depths=[2, 2, 8, 2],
        dims=96,
        drop_path_rate=0.2,
    ...
    return VSSMParaX(pretrained=pretrained, **_merge_defaults(defaults, kwargs))


def vmamba_small_parax(pretrained=None, channel_first=True, **kwargs):
    defaults = _build_vmamba_defaults(
        channel_first,
        depths=[2, 2, 15, 2],
        dims=96,
...
def vmamba_base_parax(pretrained=None, channel_first=True, **kwargs):
    defaults = _build_vmamba_defaults(
        channel_first,
        depths=[2, 2, 15, 2],
        dims=128,
...
def backbone_vmamba_tiny_parax(pretrained=None, channel_first=True, **kwargs):
    defaults = _build_vmamba_defaults(
        channel_first,
        depths=[2, 2, 8, 2],
        dims=96,
    ...
    return BackboneVSSMParaX(pretrained=pretrained, **_merge_defaults(defaults, kwargs))


def backbone_vmamba_small_parax(pretrained=None, channel_first=True, **kwargs):
    defaults = _build_vmamba_defaults(
        channel_first,
        depths=[2, 2, 15, 2],
        dims=96,
...
def backbone_vmamba_base_parax(pretrained=None, channel_first=True, **kwargs):
    defaults = _build_vmamba_defaults(
        channel_first,
        depths=[2, 2, 15, 2],
        dims=128,
