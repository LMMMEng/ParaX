...
class ParaXBlock(Block):
    def __init__(self, dim=64, depth=None, layer_id=None,
                 parax_rank=128,
                 ...
                 **kwargs):
        super().__init__(dim=dim, layer_id=layer_id, **kwargs)
    ...
    def forward(self, x, params=None):
        ...
        if self.parax_enable_conv:
            ...
            spatial_tokens = self.adapter(spatial_tokens, params, H, W)
        else:
            spatial_tokens = self.adapter(spatial_tokens, params)
        x = torch.cat([x[:, 0, None], spatial_tokens], dim=1)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        for i in range(depth):
            self.blocks.append(
                ...
                config=tuning_config, layer_id=i, depth=depth,
        ...
        self.peft_params = nn.ParameterDict(parax_params)
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            x = blk(x, params=self.peft_params)
            if self.tuning_config.vpt_on:
...
def vit_base_patch16_parax(pretrained=None, **kwargs):
    tuning_config = kwargs.pop('tuning_config', EasyDict(
        # AdaptFormer
        ffn_adapt=False,
        ffn_option="parallel",
    ...
    model = vit_base_patch16(tuning_config=tuning_config, **kwargs)
