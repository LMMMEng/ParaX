...
try:
    ...
    from mmdet.utils import get_root_logger as _get_mmdet_root_logger
except Exception:
    MMDET_BACKBONES = None
    _get_mmdet_root_logger = None

try:
    from mmseg.models.builder import BACKBONES as MMSEG_BACKBONES
    from mmseg.utils import get_root_logger as _get_mmseg_root_logger
except Exception:
    MMSEG_BACKBONES = None
    _get_mmseg_root_logger = None
...
def get_root_logger():
    if _get_mmdet_root_logger is not None:
        return _get_mmdet_root_logger()
    if _get_mmseg_root_logger is not None:
        return _get_mmseg_root_logger()
    return None
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, depth=None,
                 parax_rank=128,
        ...
        num_experts = depth
        self.adapter1 = ParaXAdapter(
            dim, parax_rank, num_experts,
    ...
    def forward(self, x, mask_matrix, params=None):
        """ Forward function.
    ...
    def forward(self, x, params, H, W):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, attn_mask, params)
            else:
                x = blk(x, attn_mask, params)
...
class SwinParaX(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in out_indices:
            ...
            layer_name = f'extra_norm{i_layer}'
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            x_out, H, W, x, Wh, Ww = layer(x, param, Wh, Ww)

            if i in self.out_indices:
                norm_layer = getattr(self, f'extra_norm{i}')
...
for _backbone_registry in (MMDET_BACKBONES, MMSEG_BACKBONES):
    if _backbone_registry is not None:
        _backbone_registry.register_module()(SwinParaX)